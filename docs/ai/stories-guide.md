...
Все стори, кроме **Playground** и **Visual tests**, выносятся в подпапку `examples/` и импортируются в основной файл. Если в основной папке находятся несколько файлов с расширением `.stories.tsx`, внутри подпапки `examples` необходимо создать подпапки по названию компонентов из файлов с расширением `.stories.tsx` и к названию примеров также добавлять название компонента - например, `AlertProcessDefaultExample.tsx`, `AlertContextDefaultExample.tsx`. Также необходимо в каждой такой подпапке создать отдельный файл `index.ts`.
