...
- **Фокус при навигации с клавиатуры.** Узел сообщает в render-функцию
  ...
  `focus()` на `<input>`. Фокус не перехватывается при монтировании и при
  работе мышью — эффект срабатывает, только если текущий `document.activeElement`
  не содержит чекбокс. Когда флаг статичности снимут, у узлов с детьми фокус
  вместо чекбокса будет получать стрелка раскрытия.
