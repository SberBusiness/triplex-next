...
- **Клавиатура на чипсе:** Enter и Space открывают и закрывают календарь с
  ...
  дропдауне закрывает его (`DatePickerExtended`). В отличие от базового `Chip`,
  внешний `onKeyDown` вызывается не на любую клавишу, а только на Enter/Space и
  только когда чипс не `disabled` (так же, как в `ChipDatePicker`). Навигация по сетке месяцев —
