...
- Читает плановый прогон `release-weekly.yml` за день, а при его отсутствии
  или пустом исходе — ручной; различает исходы: штатный
  пропуск (подготовка не завершена или выпускать нечего), полный релиз,
  авария, поломка workflow.
