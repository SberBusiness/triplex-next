...
2. **Автоматически в финале работы по задаче Linear** — ветка `TRI-XXX-...`
   ...
   В этом сценарии после коммита сразу пуш и ожидание PR от workflow
   `auto-pr.yml` (шаг 8).
...
Release notes **не нужны** для изменений только в stories / тестах / документации.
Рефакторинг без изменений для потребителя записи не лишается, но получает
**одну короткую строку** («Проведён AI-рефакторинг» + то немногое наблюдаемое,
что появилось, например `displayName`) — см. «Главное правило» в
`docs/ai/commits.md`.
...
4. Проверь запись на объём: в ней не должно быть внутренней кухни (счётчики тестов, `*-ai.md`, stories, ROADMAP, JSDoc) и перечисления того, что не изменилось. Разросшуюся до абзаца запись про рефакторинг — сожми до строки.
5. Файл release notes должен попасть в тот же коммит, что и изменение компонента.
...
- Тело — опционально, для сложных изменений. **В автоматическом сценарии
  задачи Linear тело обязательно**: из него workflow `auto-pr.yml` берёт
  описание PR (секции «Что сделано» / «Как проверить»), а из первой
  строки — заголовок PR.
...
**PR не создавай сам** — его создаст workflow `auto-pr.yml` от аккаунта
бота (заголовок и описание берутся из финального коммита). Дождись
появления PR с ретраем (~20–60 с, сниппет — в `docs/ai/commits.md`
§ «PR-воркфлоу»):
...
`finish-task` (резюме в задачу без подтверждения).
...
- **В ветках `TRI-*` не вызывай `gh pr create`** — PR создаёт workflow `auto-pr.yml`; прямое создание допустимо только как fallback при упавшем/отсутствующем прогоне workflow (см. шаг 8).
