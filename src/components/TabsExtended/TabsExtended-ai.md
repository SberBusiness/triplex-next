...
- `role="tablist"` и `role="presentation"` выставляются до спреда `...rest`, поэтому потребитель может их переопределить; `role="tab"` на `TabButton` выставляется после спреда и переопределению не поддаётся.
