...
- Обработчик клика по шапке навешивается по `expandableContent`, то есть только когда
  сворачивание реально доступно. При `disableAdaptiveCollapsing={true}` обработчика на шапке
  нет вовсе, поэтому тап по ней не дёргает `setOpen` и не переключает класс `open`.
...
- Без `IslandWidgetWrapper` `ExtraFooter` работает, но виджет о нём не узнает: контекст по
  умолчанию содержит no-op колбэки, тень не появится.
...
- Обёртка считает раскрытые подвалы, а не хранит флаг: каждый раскрытый `ExtraFooter`
  регистрируется в контексте на всё время, пока `open === true`, и снимается с учёта
  и при закрытии, и при размонтировании. Поэтому условный рендеринг подвала так же
  корректен, как управление через `open` (тень не залипает), а если в одной обёртке
  раскрыто несколько подвалов, закрытие или удаление одного не снимает тень, пока
  открыт хотя бы один. `hasExtraFooter` в контексте — это «счётчик > 0».
...
- **`box-shadow` тени `ExtraFooter` больше не помечен `!important`.** Помета снята: в библиотеке
  нет ни одного другого правила на `.islandWidgetWithExtraFooter::after`, перебивать было нечего.
  Возвращать помету не нужно — если тень вдруг начнёт перебиваться, ищи появившееся
  конкурирующее правило, а не возвращай `!important`.
