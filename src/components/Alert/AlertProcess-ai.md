...
| `...HTMLDivElementAttributes` | —                         | —            | Уходят на элемент раскрываемого контента, не на обёртку спойлера     |
...
- **`forwardRef`** — обязателен и у `AlertProcess` (target — корневой
  `HTMLDivElement`), и у `AlertProcess.Spoiler` (target — элемент раскрываемого
  контента, тот же, на который уходит `...rest`). Не убирать.
...
- **`...rest` и `ref` у `AlertProcess.Spoiler` уходят не на внешнюю обёртку, а
  на внутренний элемент раскрываемого контента** — тот, что получает классы
  `expandableContent` / `expanded`. Внешняя обёртка спойлера — структурный
  `<div>` без класса. `className` с 1.42.0 **мерджится** к собственным классам
...
- **`aria-expanded` / `aria-controls` на кнопке спойлера не выставляются** —
  состояние раскрытия передаётся только визуально (поворот каретки и
  `max-height`). Скрытый контент при этом остаётся в DOM и доступен скринридеру
  всегда, независимо от `open`.
