...
потребитель — типовой приём с `MobileView` показан в story `Example: production`.
...
- Поле `UploadZone.Input` скрыто через `display: none`, поэтому в таб-порядок
  тоже не попадает.
- Отсюда обязательное требование к потребителю: в `children` должен быть
  фокусируемый элемент, вызывающий `openUploadDialog` — иначе загрузка файлов
  недоступна с клавиатуры и скринридеру. Все примеры в stories построены
  именно так.
- Подходит `Button` (в том числе `theme={EButtonTheme.LINK}`, если нужен вид
  ссылки): он рендерит `<button>` и попадает в таб-порядок. **`Link` для этого
  не годится** — без `href` он рендерит `<a role="link">`, а такой элемент
  браузер в таб-порядок не включает, и диалог выбора файлов остаётся
  недоступным с клавиатуры.
...
| `WithDropZoneContainer` | `WithDropZoneContainer.tsx` | Дроп-зона на площади внешнего контейнера |
