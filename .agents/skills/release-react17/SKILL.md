...
`1.Y.0`. Во внутреннем registry `latest` тогда уедет на `0.Y.0`, и это надо
чинить: см. «Если `latest` завис на 0.x». На npmjs он останется на React
18-сборке.
...
Скрипт [`scripts/releaseNotesMd.js`](../../../scripts/releaseNotesMd.js) —
...
Ожидается: версия `<V0>` резолвится в реестре. Сразу после run её может
ещё не быть: npm отдаёт свежую публикацию через минуты (TRI-151 — 8,5).
Прежде чем считать, что публикация не дошла, дождись индексации — цикл
«версия появилась в реестре» из [`release-react18`](../release-react18/SKILL.md#ожидание-индексации-npm)
с `<V0>`, до 15 минут.
...
skill запускали отдельно). Во внутреннем registry теги не проносятся, `latest`
там считается по последней публикации — и `npm install @sberbusiness/triplex-next`
отдаёт всем React 17-сборку. На npmjs `latest` остался на прежней React
18-сборке: `<V0>` опубликована под тегом `react17`.
...
- напоминание, что во внутреннем registry `latest` сейчас соответствует `V0`
  и вернётся на `1.Y.0` после React 18-половины — ожидаемое промежуточное
  состояние; на npmjs `latest` не двигался, `V0` встала под тег `react17`;
