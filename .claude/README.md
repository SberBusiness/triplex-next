...
└── skills/                              # каноничное место для skills (runtime-neutral)
    ...
    ├── release-auto/SKILL.md              # отчёт по итогам еженедельного релиза
...
ROADMAP. По задаче Linear (`TRI-XXX`) после зелёного ревью сам коммитит и
пушит; PR создаёт workflow `auto-pr.yml` от аккаунта бота (агент дожидается
его появления). Вне задачи Linear — не коммитит.
...
ревью) — в этом сценарии после коммита сразу пушит; PR создаёт workflow
`auto-pr.yml` от аккаунта бота, skill дожидается его появления
(см. `docs/ai/commits.md` § «PR-воркфлоу»). Вне задачи Linear не пушит.

---

### `/release`

Оркестратор **ручного** релиза: «Выпусти релиз X.Y.Z». Еженедельный релиз
делает `release-weekly.yml` в GitHub Actions по расписанию, скилы в нём не
участвуют. Выпускаются только минорные версии; запускать локально — из
облака прокси не даст опубликовать.
...
- Ждёт зелёный CI, мержит PR (мерж в `main` ограничен branch protection:
  при отказе политики мержит человек из restrictions, `--admin`), создаёт
  GitHub Release с notes из MDX (`node scripts/releaseNotesMd.js`) и
  пометкой latest.
...
- **Перелитие `main` → `release-0` не делает** — это ручной шаг мейнтейнера
  ...
  зелёный.
- Гейты `test-unit` / `build` — обязательны: релизный
...
- После публикации `latest` во внутреннем registry временно соответствует
  `0.Y.0` — его возвращает React 18-половина. На npmjs `latest` не двигается:
  `0.Y.0` уходит под тегом `react17` (TRI-146).

---

### `/release-auto`

Отчётный skill для еженедельной cloud-routine (пятница, ~14:00). **Ничего не
выпускает** — релиз целиком делает `release-weekly.yml` по расписанию GitHub.

- Ищет прогон `release-weekly.yml` за день: сначала плановый, а если его
  нет или он вышел пустым — ручной (расписание опаздывает, и релиз в такие
  недели выпускается через `workflow_dispatch`).
- Различает исходы: штатный пропуск (подготовка не завершена или выпускать
  нечего), полный релиз, авария, поломка workflow.
- Сверяет версии веток, `dist-tags.latest`, наличие ассета
  `mcp-data-<версия>.json` в релизе.
- Публикует резюме в задачу Linear; если релиза не было — задачу не заводит.
- Аномалии называет первой строкой: `latest` на `0.x` означает, что
  `npm install` отдаёт всем React 17-сборку.

Почему релиз не здесь: прокси облачных сессий режет `gh release create`,
`gh workflow run` и push тегов — см. `docs/ai/commits.md`, «Облачные сессии».
