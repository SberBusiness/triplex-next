...
| 2026-09-03 | AI-рефакторинг (TRI-61): JSDoc на props, контексте и всех частях выпадающего блока; значение контекста мемоизировано, обработчик `onMouseDown` стабилизирован; убрана избыточная пересборка `mobileViewProps` с комментарием, почему объект передаётся всегда; unit-тесты разбиты по субкомпонентам и расширены с 1 до 45 кейсов. Публичный API и наблюдаемое поведение не изменены. |
