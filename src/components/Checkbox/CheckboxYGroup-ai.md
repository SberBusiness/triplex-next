...
- Вертикальный отступ задаётся правилами вида `.label.md + .label.md`, поэтому его получают только соседние `Checkbox` одного размера. На мобильном брейкпоинте величина отступа одинакова для всех размеров, но по-прежнему применяется только между соседями одного размера.
