...
| `Tooltip.XButton` | Нет | Кнопка закрытия. Её наличие добавляет корню класс `closable` (отступ справа у контента). В адаптивном режиме кнопка закрытия есть всегда, независимо от `Tooltip.XButton`, — он лишь передаёт ей свои props (в том числе `aria-label`) |
...
`ITooltipProps` расширяет `React.HTMLAttributes<HTMLDivElement>`. Неизвестные компоненту атрибуты уходят на корневой `<div>` десктопной подсказки (и на контейнер мобильного оверлея в адаптивном режиме). Исключение — `onKeyDown`: см. «Ограничения».
...
- `onKeyDown`, переданный в `Tooltip`, на десктопе до корневого элемента подсказки не доходит: `TooltipDesktopBase` вырезает его из props и никуда не прокидывает. В адаптивном режиме он, наоборот, попадает на контейнер оверлея — поведение одного и того же prop в двух режимах различается. Клавиатурный обработчик вешай на элемент внутри `Tooltip.Target`.
...
- Мобильная версия — оверлей `DropdownMobile`; кнопке закрытия `aria-label` задаёт потребитель через props `Tooltip.XButton`. Кнопка там рендерится всегда, поэтому без `Tooltip.XButton` она остаётся без доступного имени — в адаптивном сценарии `Tooltip.XButton` с `aria-label` стоит передавать, даже если на десктопе крестик не нужен.
...
| `VisualTests` | `VisualTests.tsx` | `alignTip: start`, `alignTip: end`, широкая подсказка с длинным текстом, ссылкой и кнопкой закрытия |
...
| 2026-08-03 | Правки по второму ревью PR #496 (документация и stories, кода не касаются): в «Ограничения» добавлено, что `onKeyDown` на десктопе не доходит до корневого элемента подсказки, а в адаптивном режиме доходит; уточнено, что в адаптивном режиме кнопка закрытия рендерится всегда и без `Tooltip.XButton` остаётся без `aria-label`; в `VisualTests` добавлен блок с `alignTip: end` — раньше это значение enum не было покрыто ни примером, ни тестом. |
