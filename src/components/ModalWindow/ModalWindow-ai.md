...
- **`ModalWindowContent`** — контейнер контента. Оборачивает дочерние секции в `Page` (`triplex-next/Page`). Имеет `isLoading?: boolean` и `loadingTitle?: React.ReactNode` — при `isLoading` поверх контента показывается `LoaderScreen` типа `middle`; текст под спиннером передаётся в `LoaderScreen` через prop `description`. Свой `displayName`. Свои props через интерфейс `IModalWindowContentProps` — экспортируется.
