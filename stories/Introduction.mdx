...
Тег `react17` указывает на актуальную React 17-сборку в npmjs. Если пакет
ставится из внутреннего registry, теги туда не проносятся — указывайте
точный номер версии (`0.Y.0`).
