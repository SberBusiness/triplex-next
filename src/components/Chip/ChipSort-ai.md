...
- **Доступное имя таргета задать нечем.** У icon-only чипса нет текста, а
  `role="combobox"` стоит на `<span>` из `ChipIcon`. Переданные потребителем
  `aria-label` / `aria-labelledby` попадают в `...restProps` и приземляются на
  корневой `<div>`, у которого нет `role`: на generic-элементе эти атрибуты
  спецификацией ARIA не поддерживаются и скринридером не озвучиваются. Добраться
  до таргета через публичный API нельзя — `targetProps` исключён `Omit`'ом, а
  `renderTarget` пользовательские props в `ChipIcon` не спредит. Чинится пробросом
  до `ChipIcon` (например, возвратом `targetProps`) — изменение публичного API,
  отдельной задачей синхронно по семейству `Chip*`.
- **Цель `ref` разошлась с объявленным типом.** `ref` уходит на чипс-таргет
  (`<span>` из `ChipIcon`), а тип объявлен `HTMLDivElement` — компилируется только
  из-за структурной совместимости. Соседний `ChipSelect` прокидывает `ref` на
  корневой `<div>` и своему типу соответствует, то есть внутри семейства контракт
  разъехался. Кандидат на исправление в мажорной версии, не вечное правило.
...
- **`forwardRef`** — не убирать. `ref` указывает **не** на корневой `<div>`, а на
  чипс-таргет (`<span>` из `ChipIcon`). Цель `ref` и объявленный тип не менять без
  breaking-окна — почему они разошлись, см. «Известные ограничения».
...
- Компонент **не задаёт себе доступное имя**, и задать его снаружи сейчас нечем —
  см. «Известные ограничения». Библиотека мультиязычная, текст не хардкодится.
