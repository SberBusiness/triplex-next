...
- Клик по кнопке очистки (`onClear`) не раскрывает выпадающий блок: `SelectExtendedFieldTarget` останавливает всплытие в обработчике `FormFieldClear`. Без этого событие дошло бы до `onClick` на `FormField` и переключило `opened` — так же гасят всплытие `Chip.Select` и `Chip.DatePicker`. Не убирать: затрагивает `SelectField` и `MultiselectField.Target`, которые рендерят тот же `Target`.
...
- `setOpened`, отдаваемый рендер-функциям, — это сеттер `useState`. Он обязан оставаться стабильным между рендерами: построенные на нём компоненты кладут его в зависимости колбэков — например `Chip.Select` через `ChipSelectTarget` (`useCallback(..., [opened, setOpened, onKeyDown])`).
...
| `VisualTests` | `VisualTests.tsx` | Раскрытый список и видимая кнопка очистки (через `play`), длинное значение, загрузка в размере SM |
