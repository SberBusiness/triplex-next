...
`dist-tags.react17` на npmjs должен указывать на парную `0.Y.0` — с TRI-146
React 17-половина публикуется с явным `--tag react17`. Во внутренний registry
теги не проносятся, там всё решает порядок публикаций.
