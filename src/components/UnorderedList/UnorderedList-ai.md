...
- **`ref` элемента** можно передать прямо в объекте элемента — `UnorderedListItem` объявлен через
  `forwardRef` и получает его как обычный prop из spread.
...
- **Маркер-обёртка рендерится всегда**, даже если `marker` не передан — внутри неё оказывается либо
  переданный узел, либо точка по умолчанию. Условный рендер обёртки сломает выравнивание маркера
  по первой строке многострочного текста.
