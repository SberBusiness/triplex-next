...
export interface ITagProps extends React.HTMLAttributes<HTMLSpanElement> {
    ...
    /**
     * Уникальный идентификатор тега. Рендерится атрибутом id на корневом элементе и передаётся
     * аргументом в onEdit и onRemove, чтобы потребитель понял, какой тег группы изменили.
     * Значение должно быть уникальным в пределах документа — как у любого DOM-идентификатора.
     */
    id: string;
