...
- **Текст кнопки закрытия задаёт потребитель.** У `Confirm.Close` есть захардкоженный дефолт
  `title="Закрыть"` — для мультиязычного продукта передавай `title` (и при необходимости
  `aria-label`) явно.
