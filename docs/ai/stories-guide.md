...
- **Playground** — добавить новый prop в `args` (дефолтное значение) и в `argTypes` (контрол).
...
- **Компоненты без визуального интерфейса** — `KeyDownListener`, `ComposedKeyDownListener` и подобные обёртки, которые не рендерят собственной разметки (слушатели событий, провайдеры контекста). На скриншот попадает только обвязка примеров, поэтому регрессию компонента такой тест поймать не может, а перегенерации требует от любой правки примера. Набор исключается **целиком**: `testRunner: { skip: true }` ставится в `parameters` у `meta`, отдельные стори параметр не дублируют, стори `VisualTests` не создаётся, файлов в `__screenshots__/` быть не должно. Поведение таких компонентов покрывается unit-тестами.
...
- [ ] Компонент действительно рендерит собственную разметку. Если нет (слушатель событий, провайдер) — весь набор исключён через `testRunner: { skip: true }` в `meta`, `VisualTests` нет, baseline-файлов нет; остальные пункты этого раздела не применяются
