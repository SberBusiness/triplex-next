...
**Куда что уходит.** `className` мерджится в корневой `<div>` карусели, а неизвестные компоненту HTML-атрибуты из `...rest` — на `<ol role="tablist">` внутри неё. Это разные элементы: `aria-label`, `data-*` и прочие атрибуты оказываются на списке шагов, а не на корне.
...
|---|---|---|---|
| `size` | `EComponentSize` | `LG` | Один prop на весь компонент: задаёт и высоту шагов, и размер кнопок прокрутки |
...
- Подводка выбранного шага в видимую область (`alignStep`) — чистые функции модульного уровня в `Stepper.tsx`, зафиксированы unit-тестами: на узких экранах шаг центрируется, на широких — прижимается к ближайшему краю вместе с соседним шагом.
