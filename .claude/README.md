...
└── skills/                              # каноничное место для skills (runtime-neutral)
    ...
    ├── release-react18/SKILL.md           # релиз 1.Y.0 из main (npm-тег latest)
    ├── release-react17/SKILL.md           # релиз 0.Y.0 из release-0 (npm-тег react17)
...
```text
1.Y.0  →  /release-react18   (main,      npm-тег latest)
0.Y.0  →  /release-react17   (release-0, npm-тег react17)
```
...
- Если 18-часть остановилась — 17-часть **не запускает**: та опирается на уже
  опубликованную `1.Y.0` и её notes в `main`.
...
React 18-релиз `1.Y.0` (npm-тег `latest`) из `main`.
...
Парный React 17-релиз `0.Y.0` (npm-тег `react17`) из `release-0`.
Запускается после подтверждённой публикации `1.Y.0`.
...
- Проверяет, что `dist-tags.react17` обновился, а `latest` остался на `1.Y.0`.
