...
/** Свойства компонента StatusTrackerButton. Тема LINK недоступна: блочный режим для неё не поддерживается. */
export type IStatusTrackerButtonProps = Exclude<TButtonProps, IButtonLinkProps>;

/**
 * Компонент кнопки для футера статус-трекера.
 * Блочный режим включён по умолчанию; `block` стоит до `...rest`, поэтому потребитель может его переопределить.
 */
