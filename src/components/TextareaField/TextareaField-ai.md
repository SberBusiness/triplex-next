...
- **Поле не растёт по содержимому.** У `<textarea>` выставлен `resize: none`, автоувеличения высоты нет — размер области ввода задаёт только `size` (`min-height` 64px для SM, 60px для MD и LG). Нужна другая высота — задавай её через `textareaProps.rows` или собственный `className`.
