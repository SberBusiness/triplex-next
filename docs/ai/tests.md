...
**Скриншот-тесты обязательны для любого компонента с видимым UI.** По умолчанию все не-Playground stories автоматически снимаются на двух viewport'ах (xs/xl) — это базовое покрытие. Если оно недостаточно, создавай отдельную `VisualTests` story.

Создавай `VisualTests` story если:
- Компонент требует взаимодействия перед скриншотом (открыть dropdown, hover, focus, открыть модалку/попап) — **обязательно** для overlay-компонентов (Modal, Tooltip, Popover, Dropdown, LightBox).
- Нужно покрыть состояние, которого нет в документационных stories.
- Нужно объединить несколько вариантов в один скриншот.

После добавления новых stories обязательно сгенерируй baseline-скриншоты через CI workflow — без baseline скриншот-теста фактически нет, и regression не поймается.
