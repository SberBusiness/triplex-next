...
- **Корневой элемент — `<div role="status">`.** На эту связку опираются unit-тесты компонента и
  тесты потребителей (`LoaderScreen.test.tsx`, `TableBasic.test.tsx`), которые ищут лоадер через
  `getByRole("status", { name: "loading" })`. Имена CSS-классов снаружи не используются —
  тесты потребителей намеренно опираются только на роль и доступное имя.
