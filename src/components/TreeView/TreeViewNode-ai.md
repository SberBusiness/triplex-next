...
Промаха по id компонент не сигнализирует, но два случая промаха ведут себя
по-разному. Ноду с указанным id ищут **в поддереве текущего родителя**, а не по
всему дереву.

- **Id не найден** — нода добавляется в конец своего уровня, то есть в порядке
  рендера. Детерминированно и безопасно; это штатный режим, а не ошибка.
  Именно так работает `nextNodeId` в `CollapsibleTree` и `CheckboxTree`: они
  передают id следующей ноды из массива данных, а та на момент конструктора
  текущей ещё не зарегистрирована — промах и откат в порядок рендера здесь
  происходят всегда.
- **Id найден, но это не прямой ребёнок того же родителя** — позиция
  определяется индексной арифметикой `splice` в `AbstractTreeNode.addChild`.
  Это **деталь текущей реализации, а не контракт**: тестами не закреплена,
  класс общий для всех абстрактных деревьев, полагаться нельзя.

Правильное употребление одно: ссылаться на уже зарегистрированную соседнюю ноду
того же уровня.
...
- **`role="treeitem"` и `aria-expanded` выставляются после `{...props}`** —
  ...
  семантика, и потребитель при желании может задать его сам. Но переданный
  `tabIndex` побеждает навсегда: `TreeView.updateTabIndexNodes` продолжит
  пересчитывать значение в абстрактной ноде, на DOM это уже не повлияет. Поставив
  `tabIndex={0}` всем нодам, потребитель получит не свой roving tabindex, а
  дерево, где каждая нода — отдельная остановка Tab. Переопределять — только если
  берёшь управление фокусом целиком на себя.
...
- **`children` — функция, а не `ReactNode`.** Смена сигнатуры — breaking change
  для всей цепочки потребителей. Напрямую `TreeView.Node` рендерит только
  `CollapsibleTreeExtended`; `CollapsibleTree` строится поверх него, а
  `CheckboxTree` доходит до ноды через `CheckboxTreeExtended` →
