...
в пятницу 13:00 МСК, routine идёт следом в тот же день.

Оба фильтра обязательны, каждый закрывает свою ошибку:

- `--event=schedule` — иначе поймаешь более поздний ручной прогон,
  в частности `dry_run`, у которого `preflight` успешен, а остальные джобы
  `skipped`. Выглядит один в один как «релиза не было штатно»;
...
Список пуст — так и скажи: расписание не сработало, это повод проверить
`release-weekly.yml`, а не выпускать релиз руками. Отчёт по вчерашнему
прогону в этом случае не годится.
