...
| `isLoading` | `boolean` | — | Состояние загрузки: деактивирует focus trap; `LightBox.Content` показывает `LoaderScreen` |
...
- Пока лайтбокс смонтирован, на `document.documentElement` висит класс блокировки скролла;
  снимается в cleanup (с задержкой-фиксом двойного монтирования при переключении роутов).
...
- **Клавиатура**: Esc — клик по `Controls.Close`; ArrowLeft / ArrowRight — клик по
  ...
  При открытом `SideOverlay` клавиатурные триггеры контролов лайтбокса не активны
  (рендерится вариант кнопок без триггера).
