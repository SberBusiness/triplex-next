...
export default {
	...
	parameters: {
		docs: {
			description: {
				component: "ThemeProvider - компонент для управления темами в приложении. Позволяет переключаться между светлой и темной темами, а также переопределять дизайн-токены.",
			},
		},
	},
	argTypes: {
...
export const ThemeSwitcher: StoryObj<typeof ThemeProvider> = {
	...
	render: (args) => {
		...
		const handleThemeToggle = () => {
			setCurrentTheme(currentTheme === ETriplexNextTheme.LIGHT ? ETriplexNextTheme.DARK : ETriplexNextTheme.LIGHT);
		};
