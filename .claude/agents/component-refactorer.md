...
5. **Что осталось пользователю** — если у компонента есть `{Name}-ai.md`, обновить его; написать release notes (нужны всегда: изменение публичного API или поведения — по существу, чистый рефакторинг — одной строкой), коммит.
