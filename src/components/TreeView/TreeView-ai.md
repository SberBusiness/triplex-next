...
- **Клавиатура:** `ArrowDown` / `ArrowUp` (по `event.keyCode`) переносят
  ...
  Стрелки спускаются только в раскрытые (`opened`) ветки — кроме одного
  исключения, см. ниже.
- **Известное отклонение (pre-existing):** `ArrowUp` с самой первой ноды дерева
  уходит на последнего потомка последней ветки, не проверяя её `opened`
  (`TreeViewAbstractNodeUtils.getPrevNode`, ветка «предыдущей ноды на уровне
  нет»). Если последняя ветка свёрнута, активной становится скрытая нода.
  `ArrowDown` в симметричной ситуации внутрь свёрнутой ветки не заходит.
  Поведение существует с момента появления компонента и в рамках TRI-100
  намеренно не менялось — это изменение наблюдаемого поведения, решение
  за владельцем. Зафиксировано тестом «Первая нода дерева: предыдущая -
  последняя нода дерева».
