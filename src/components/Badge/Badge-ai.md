...
- Текстовый `Badge` доступен как есть: содержимое читается в потоке. Отдельная
  подпись не нужна.
- **`Badge.Dot` не имеет доступного имени** — это чисто визуальный индикатор.
  Корневой `<span>` без роли попадает в `generic`, а для него доступное имя
  запрещено, поэтому одного `aria-label` недостаточно. Если точка несёт смысл
  («есть непрочитанное»), потребитель обязан передать роль, поддерживающую имя,
