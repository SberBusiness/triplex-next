...
`Default`, `CustomMarkerText` и `Sizes` исключены из скриншот-тестов (`testRunner: { skip: true }`) —
они рендерят то же самое, что одноимённые истории `UnorderedListExtended` (baseline совпадали
побайтово). Собственное визуальное покрытие компонента даёт `VisualTests`; выравнивание маркера
по размерам типографики покрыто скриншотом `UnorderedListExtended` → `Sizes`, там же живут
и стили маркера.
