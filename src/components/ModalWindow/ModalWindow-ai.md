...
- **Focus trap.** Активная ловушка фокуса (`focus-trap-react`) включается синхронно с `isOpen`. По умолчанию: `allowOutsideClick: true` (клик за пределами не сбрасывает trap, но не блокируется), `preventScroll: true` (фокус не скроллит layout), `fallbackFocus` указывает на сам `role="dialog"` узел (у него `tabIndex={-1}`). Расширить настройки можно через `focusTrapProps.focusTrapOptions`.
- **Почему нужен `fallbackFocus`.** Ловушка активируется синхронно при открытии. Если в этот момент внутри dialog ещё нет ни одного tabbable-узла (контент грузится асинхронно, показан лоадер, `closeButton` отрисуется тиком позже), `focus-trap` без fallback падает с `"must have at least one container with at least one tabbable node"`. `fallbackFocus` на dialog снимает это падение — фокус временно встаёт на сам контейнер.
...
- **`ModalWindowContent`** — контейнер контента. Оборачивает дочерние секции в `Page` (`triplex-next/Page`). Имеет `isLoading?: boolean` и `loadingTitle?: string` — при `isLoading` поверх контента показывается `LoaderScreen` типа `middle`; текст под спиннером передаётся в `LoaderScreen` через prop `description`. Свой `displayName`. Свои props через интерфейс `IModalWindowContentProps` — экспортируется.
...
| `LoadingState` | `LoadingState.tsx` | Состояние загрузки (`isLoading=true` + `loadingTitle` как строка) |
