...
- Положительные маргины навешиваются селектором `.indent-{n} .label`, то есть только на потомков с корневым классом `Checkbox`. Для произвольных children (текст, свой контрол, обёртка без `Checkbox` внутри) отступ не применится, а отрицательные маргины контейнера останутся — контент выедет за границы группы.
