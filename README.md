...
npm run test-visual:gallery # Запуск генерации gallery.html с информацией об изменениях. Запускать после npm run test-visual, как в test-visuals будут упавшие тесты.
npm run test-visual:serve # Просмотр изменений - http://127.0.0.1:8080/gallery.html. Запускать после npm run test-visual:gallery.
```
