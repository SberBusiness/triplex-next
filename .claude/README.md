...
Оркестратор релиза. Запускается по фразе «Выпусти релиз X.Y.Z» (номер версии
обязателен). Выпускаются только минорные версии.
...
- Перелитие `main` → `release-0` **напрямую, без PR**, с разрешением
  конфликтов по правилам: логика из `main`, React 17-совместимость
  (версии зависимостей, полиморфные типы, типовая обвязка) из `release-0`.
- Локальные гейты `tsc` / `test-unit` / `build` — обязательны, потому что
  `ci.yml` триггерится только на `pull_request` и прямой push в `release-0`
  не проверяет.
- Переносит тело release notes `v1/1.Y.0.mdx` → `v0/0.Y.0.mdx` (меняются
  только заголовки), создаёт заготовку на `0.(Y+1).0`.
