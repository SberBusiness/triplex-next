...
Не используй когда: нужен футер страницы — там `Page.Footer` (`FooterPage`), он в зависимости
от обязательного `type` оборачивает контент в карточку (`Island`) и умеет прилипать к нижней
границе экрана (`EFooterPageType.FIRST`) либо рендерит голый `Footer` (`EFooterPageType.SECOND`).
...
- Тип props у `FooterDescriptionControls` — `IFooterDescriptionControlsProps` — импортируется
  ...
  с компонентом. Переносить и переименовывать его нельзя — это публичный API `TableBasic`
  и breaking change. Правки внутри интерфейса, не меняющие его тип (например JSDoc на
  `children`, как у остальных трёх интерфейсов семейства), допустимы — это техдолг,
  который закрывается вместе с задачей по `TableBasic`.
