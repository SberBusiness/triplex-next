...
- [ ] В `*-ai.md` токены указаны путями `{Группа}.{Токен}`, а не css-переменными.
      Проверка по изменённым файлам:
      `npm run syncAiMdTokens -- --check <пути к изменённым *-ai.md>`.
      Без путей команда проверяет все AI.md и до завершения миграции красная —
      это ожидаемо и не относится к ревьюируемым изменениям.
