...
- `disabled` и `readOnly` ведут себя по-разному и взаимозаменяемыми не являются:
  `disabled` выключает поле целиком — оно не попадает в таб-обход и не получает фокус;
  `readOnly` фокус получает и остаётся в таб-обходе, но не даёт изменить значение.
  Визуально оба состояния не отличаются от обычного — различие обеспечивает потребитель.
...
| `VisualTests` | `VisualTests.tsx` | Скриншот-регрессия: плейсхолдер, заполненное, disabled, readOnly, переполнение, растягивание по ширине, инлайн-выравнивание |
