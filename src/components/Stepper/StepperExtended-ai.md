...
|---|---|---|---|
| `size` | `EComponentSize` | `EComponentSize.MD` | Размер шагов: SM / MD / LG. Уходит в контекст, каждый шаг берёт его оттуда |
...
| 2026-09-24 | `VisualTests` приведена к тому же виду, что и `States`: иконки убраны, порядок шагов «пройден → выбран → недоступен → не пройден». Иконки остались только в `WithIcons`. Код компонента не менялся. |
| 2026-09-25 | Размер по умолчанию изменён с `LG` на `MD` (TRI-84) — вслед за `Stepper`, дефолт хранится и в пропе, и в значении контекста по умолчанию. Изменение наблюдаемое: без явного `size` шаги становятся меньше, описано в release notes 1.48.0. |
