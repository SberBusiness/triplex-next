...
| `selected` | `boolean` | — | Состояние выбора в контролируемом режиме. Само наличие prop'а при монтировании включает контролируемый режим |
...
- **Неконтролируемый** (`selected` не передан): состояние выбора хранится внутри. Клик,
...
- **Контролируемый** (`selected` передан): компонент сам состояние не меняет. Клик,
  `Enter` и `Space` вызывают `toggle(!selected)` — потребитель обязан обновить `selected`,
  иначе вид карточки не изменится. `onToggle` в этом режиме вызывается **не по клику**, а
