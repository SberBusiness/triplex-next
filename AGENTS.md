...
| `.agents/skills/release-auto/SKILL.md` | Reporting on the weekly release run; the release itself runs in GitHub Actions (`release-weekly.yml`) |
...
- No hardcoded colors — use CSS variables from design tokens (except for `transparent` for resetting default browser colors)
