...
сгенерированном коде недопустимы. Цвета задаются css-переменными
дизайн-токенов:

```css
color: var(--triplex-next-Link-Text_Color_Default);
```

Имена переменных не придумываются: токен сначала заводится в TS-объекте
дизайн-токенов пакета (`src/components/DesignTokens/`), css-переменные
генерируются из него (версия пакета дописывается к имени при сборке npm).
Существующие токены — в `get_tokens` MCP-сервера; переопределение значений —
через prop `tokens` у `ThemeProvider` (путь вида `Группа.Токен`).
