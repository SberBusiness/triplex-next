import React, { useState } from "react";
import { EComponentSize, SliderRange, TSliderRangeValues } from "@sberbusiness/triplex-next";

/** Возвращает значения от from до to (не включая), с шагом step. */
const range = (from: number, to: number, step: number): number[] => {
    const values: number[] = [];

    for (let value = from; value < to; value += step) {
        values.push(value);
    }

    return values;
};

/**
 * Шкала сумм с неравномерным шагом: чем больше сумма, тем крупнее шаг.
 * Слайдер работает с индексами шкалы, поэтому шаги равны визуально.
 */
const AMOUNTS = [
    0,
    10000,
    ...range(20000, 240000, 20000),
    ...range(240000, 1000000, 40000),
    ...range(1000000, 2150000, 50000),
    ...range(2150000, 2600000, 75000),
    ...range(2600000, 3000000, 100000),
    ...range(3000000, 3800000, 200000),
    ...range(3800000, 9000000, 400000),
    ...range(9000000, 10000000, 500000),
    ...range(10000000, 25000000, 2500000),
    ...range(25000000, 40000000, 5000000),
    ...range(40000000, 90000001, 10000000),
    100000000,
];

const amountFormat = new Intl.NumberFormat("ru-RU");

const MARKS = [
    { value: 0, label: "0 млн" },
    { value: AMOUNTS.indexOf(1000000), label: "1 млн" },
    { value: AMOUNTS.indexOf(3000000), label: "3 млн" },
    { value: AMOUNTS.length - 1, label: "100 млн" },
];

export const Amounts = () => {
    const [amountIndexes, setAmountIndexes] = useState<TSliderRangeValues>([
        AMOUNTS.indexOf(1000000),
        AMOUNTS.indexOf(3000000),
    ]);

    return (
        <div style={{ maxWidth: "750px", padding: "30px" }}>
            <div>
                от {amountFormat.format(AMOUNTS[amountIndexes[0]])} до {amountFormat.format(AMOUNTS[amountIndexes[1]])}
            </div>
            <br />
            <SliderRange
                min={0}
                max={AMOUNTS.length - 1}
                size={EComponentSize.MD}
                marks={MARKS}
                values={amountIndexes}
                onChange={setAmountIndexes}
                renderTooltipContent={(value) => amountFormat.format(AMOUNTS[value])}
            />
        </div>
    );
};
