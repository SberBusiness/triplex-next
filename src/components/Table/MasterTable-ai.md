...
- Значение контекста мемоизировано (`useMemo` по `columns` и `loading`). Возврат к новому объекту на каждый рендер снова заставит `useEffect` в `TableBasic` перезапускаться на каждый рендер контейнера — не откатывать. Обратная сторона: `TableBasic` пересинхронизирует контекст только при смене ссылки на свой `columns`, поэтому потребитель обязан заменять массив колонок иммутабельно (`map`/спред, а не мутация на месте) — мутация той же ссылки до `ColumnSettings` не дойдёт.
