...
describe("UnorderedList", () => {
    ...
    it("renders default fallback marker when marker prop is not provided", () => {
        const itemsWithoutCustomMarker = [{ key: "fallback-item", children: "Item without custom icon" }];
        const { container } = render(<UnorderedList items={itemsWithoutCustomMarker} />);

        const listItem = screen.getByRole("listitem");
        expect(listItem).toBeInTheDocument();

        const fallbackMarker = container.querySelector("span");
        expect(fallbackMarker).toBeInTheDocument();
        expect(fallbackMarker?.className).toContain("marker");
    });
    ...
    it("maintains component composition for sub-components", () => {
