...
- В unattended публикация зависит от `PUBLISH_MODE` (зонд `release-auto`):
  `full` (локальный запуск, право на диспатч подтверждено) — через
  `gh workflow run release.yml`; `prepare` (облако — прокси запрещает все
  пути запуска релиза, TRI-117) — остановка с готовой командой для человека.
...
- В unattended публикация зависит от `PUBLISH_MODE`: `full` — через
  `gh workflow run release.yml`; `prepare` (облако — прокси запрещает все
  пути запуска релиза, TRI-117) — подготовка доводится до точки невозврата,
  остановка с готовой командой публикации для человека.
...
- До первого коммита зондирует режим публикации: право на диспатч есть →
  `PUBLISH_MODE=full`, полный релиз; 403 (штатно для облака) →
  `PUBLISH_MODE=prepare`, прогон только готовит `0.Y.0`, публикацию
  завершает человек по командам из отчёта.
