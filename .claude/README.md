...
`/release-auto`, которая выводит версию из ветки-маркера сама. Выпускаются
...
- Ветка-маркер `prerelease-1.Y.0` на origin — переключатель «релизим» и
  источник версии. Ветки нет → штатный выход, релиза на этой неделе нет.
- Предполёт: чистое дерево, версии не выпущены, миноры веток совпадают,
  notes не заготовка, `is-ancestor` проходит, CI на вершине `release-0`
  зелёный. Любая проверка не сошлась → стоп с отчётом, ничего не публикуется.
