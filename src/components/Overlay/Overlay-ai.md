...
| `onOpen` | `() => void` | — | Вызывается по завершении анимации открытия. Если оверлей смонтирован уже с `opened={true}`, анимации нет и колбэк вызывается сразу на маунте |
...
### Монтирование сразу с `opened={true}`

Оверлей, смонтированный уже открытым, никакой анимации открытия не проигрывает —
панель отрисована в конечном положении, `transitionend` не наступит. Поэтому цикл
открытия считается завершённым сразу: на маунте вызывается `onOpen`, а `onOpening`
— нет (сообщать о начале анимации нечего). Флаги `opening` / `closing` при этом
остаются `false`.

Вызов сделан в пассивном `useEffect`, а не в layout-эффекте: к моменту пассивных
эффектов refs уже проставлены, и потребитель может обратиться к DOM внутри
`onOpen`. Измерять геометрию в этот момент всё же не стоит — оверлей мог быть
смонтирован внутри контейнера, который сам ещё выезжает (`TopOverlay` поэтому
пропускает пересчёт позиции на маунте).

Монтирование с `opened={false}` не вызывает ничего — это нейтральное начальное
состояние, а не закрытие.

> Ранее (до 2026-08-27) на первом рендере гасились все колбэки: оверлей,
> смонтированный открытым, не сообщал об этом потребителю. Обёртки, которые
> в `onOpen` включают ловушку фокуса или считают позицию (`TopOverlay`), в этом
> случае оставались недоинициализированными.
